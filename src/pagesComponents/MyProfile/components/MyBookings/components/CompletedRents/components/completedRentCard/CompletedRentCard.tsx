...
const CompletedRentCard: FC<CardFeedbackProps> = ({ pictureSrc, title, seller, dateStart, dateEnd, id }) => {
  const { t } = useTranslation('ui', { keyPrefix: 'cards' });
  const { push } = useRouter();

  const redirectOnRentPage = () => {
    push({ pathname: Routes.completedRent, query: { id } });
  };
...
export default CompletedRentCard;
